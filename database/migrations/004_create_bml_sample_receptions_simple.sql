-- Migration: 004_create_bml_sample_receptions_simple.sql
-- Create BML_SAMPLE_RECEPTIONS table for Sample Reception module

-- Drop table if exists (ignore error if not exists)
BEGIN
    EXECUTE IMMEDIATE 'DROP TABLE BML_SAMPLE_RECEPTIONS CASCADE CONSTRAINTS';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -942 THEN
            RAISE;
...
-- Insert sample data (using SYS_GUID() for Oracle)
INSERT INTO BML_SAMPLE_RECEPTIONS (ID, RECEPTION_CODE, SAMPLE_TYPE_ID, RECEPTION_DATE, SEQUENCE_NUMBER) VALUES 
(SYS_GUID(), 'BLOOD.20241024.0001', (SELECT ID FROM BML_SAMPLE_TYPES WHERE TYPE_CODE = 'BLOOD' AND ROWNUM = 1), DATE '2024-10-24', 1);

INSERT INTO BML_SAMPLE_RECEPTIONS (ID, RECEPTION_CODE, SAMPLE_TYPE_ID, RECEPTION_DATE, SEQUENCE_NUMBER) VALUES 
(SYS_GUID(), 'BLOOD.20241024.0002', (SELECT ID FROM BML_SAMPLE_TYPES WHERE TYPE_CODE = 'BLOOD' AND ROWNUM = 1), DATE '2024-10-24', 2);

INSERT INTO BML_SAMPLE_RECEPTIONS (ID, RECEPTION_CODE, SAMPLE_TYPE_ID, RECEPTION_DATE, SEQUENCE_NUMBER) VALUES 
(SYS_GUID(), 'URINE.20241024.0001', (SELECT ID FROM BML_SAMPLE_TYPES WHERE TYPE_CODE = 'URINE' AND ROWNUM = 1), DATE '2024-10-24', 1);

INSERT INTO BML_SAMPLE_RECEPTIONS (ID, RECEPTION_CODE, SAMPLE_TYPE_ID, RECEPTION_DATE, SEQUENCE_NUMBER) VALUES 
(SYS_GUID(), 'STOOL.20241024.0001', (SELECT ID FROM BML_SAMPLE_TYPES WHERE TYPE_CODE = 'STOOL' AND ROWNUM = 1), DATE '2024-10-24', 1);

INSERT INTO BML_SAMPLE_RECEPTIONS (ID, RECEPTION_CODE, SAMPLE_TYPE_ID, RECEPTION_DATE, SEQUENCE_NUMBER) VALUES 
(SYS_GUID(), 'SALIVA.20241024.0001', (SELECT ID FROM BML_SAMPLE_TYPES WHERE TYPE_CODE = 'SALIVA' AND ROWNUM = 1), DATE '2024-10-24', 1);

COMMIT;

PROMPT 'BML_SAMPLE_RECEPTIONS table created successfully with sample data';
